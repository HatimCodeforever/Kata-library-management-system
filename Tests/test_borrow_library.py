...
class TestLibraryManagementBorrow(unittest.TestCase):
    def setUp(self):
    ...
    def test_borrow_book_valid(self):
        ...
        book = Book(id="12345678901", title="Available Book", author="Author", year=2022, available=True)
        self.session.add(book)
        self.session.commit()
        borrowed_book = self.session.query(Book).filter_by(id="12345678901").first()
        borrowed_book.available = False
        self.session.commit()

        # Verify the book is marked as borrowed
        updated_book = self.session.query(Book).filter_by(id="12345678901").first()
    ...
    def test_borrow_book_not_available(self):
        ...
        book = Book(id="12345678901", title="Already Borrowed Book", author="Author", year=2022, available=False)
        self.session.add(book)
        self.session.commit()
